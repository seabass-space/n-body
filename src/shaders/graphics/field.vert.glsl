...
layout (std430, set = 0, binding = 0) readonly buffer Positions { vec2 positions[][LINE_LENGTH]; };
...
void main() {
    ...
    float alpha = (brightness / 2.0) * (1.0 - float(gl_VertexIndex) / float(LINE_LENGTH));
